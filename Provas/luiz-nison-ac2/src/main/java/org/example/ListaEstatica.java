...
public class ListaEstatica {
    ...
    public void adicionaNoIndice(int indice, int valor) {
        if (indice < 0 || indice >= this.nroElem + 1) {
            System.out.println("Indice invalido");
        } else if (this.nroElem == this.vetor.length) {
            System.out.println("Lista cheia");
        } else {
    ...
    public void ordena() {
        for (int x = 0; x < this.nroElem - 1; x++) {
            for (int y = 1; y < this.nroElem - x; y++) {
                if (this.vetor[y - 1] > this.vetor[y]) {
                    int aux = this.vetor[y - 1];

                    this.vetor[y - 1] = this.vetor[y];
                    this.vetor[y] = aux;
                }
            }
        }
    ...
    public int pesquisaBinaria(int valor) {
        int indInf = 0;
        int indSup = this.nroElem - 1;
        int meio;

        while (indInf <= indSup) {
            meio = (indInf + indSup) / 2;

            if (valor == this.vetor[meio]) {
                return meio;
            } else if (valor < this.vetor[meio]) {
                indSup = meio - 1;
            } else {
                indInf = meio + 1;
            }
